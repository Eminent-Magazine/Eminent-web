  ...
  return (
    <div className="min-h-screen bg-ink text-ivory grid place-items-center p-6">
      <form onSubmit={submit} className="w-full max-w-sm border border-ivory/15 p-8 bg-ink/60 backdrop-blur">
        <p className="eyebrow !text-gold">Eminent Magazine</p>
        ...
        <button disabled={loading} className="btn-primary w-full mt-6">{loading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Sign in"}</button>
      </form>
    </div>
  );
}
