    ...
    <div className="p-10">
      <div className="flex items-end justify-between mb-6 gap-4 flex-wrap">
        ...
        <div className="flex gap-2 items-center">
          <select value={status} onChange={(e) => setStatus(e.target.value)} className="h-10 px-3 border border-input bg-card text-sm">
          ...
          <button onClick={() => fileRef.current?.click()} className="btn-outline-gold !h-10 !px-4 !text-xs inline-flex items-center gap-2">
          ...
          <tbody>
            ...
            {users.map((u) => (
              <tr key={u._id} className="border-t border-border hover:bg-secondary/40 cursor-pointer" onClick={() => setSelected(u)}>
                ...
                <td className="px-4 py-3"><StatusChip s={u.status} /></td>
                <td className="px-4 py-3 text-xs uppercase tracking-widest text-muted-foreground">{u.paymentStatus ?? "—"}</td>
                <td className="px-4 py-3 text-right">
                  <div className="inline-flex gap-1" onClick={(e) => e.stopPropagation()}>
                    <button title="Approve" onClick={() => approve.mutate(u._id)} className="w-8 h-8 grid place-items-center border border-input hover:border-primary hover:text-primary"><Check className="w-4 h-4" /></button>
                    <button title="Reject" onClick={() => { const n = prompt("Rejection notes:") ?? ""; if (n) reject.mutate({ id: u._id, notes: n }); }} className="w-8 h-8 grid place-items-center border border-input hover:border-destructive hover:text-destructive"><X className="w-4 h-4" /></button>
                  </div>
                </td>
              </tr>
            ))}
...
function StatusChip({ s }: { s?: string }) {
  const cls = s === "approved" ? "bg-primary/15 text-primary" : s === "rejected" ? "bg-destructive/15 text-destructive" : "bg-muted text-muted-foreground";
    ...
    <div className="fixed inset-0 z-50 bg-ink/60 backdrop-blur-sm flex justify-end" onClick={onClose}>
      <div className="w-full max-w-md bg-background border-l border-border p-6 overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-6 flex gap-2">
          <button onClick={() => save.mutate()} disabled={save.isPending} className="btn-primary flex-1">{save.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : "Save"}</button>
